...
**Category 5 — Pipeline-bypass risk in the rewrite path (not observed, architecturally present).** `_restricted_disclosure_hit` runs first in `decide()` and returns immediately on a match — either a `REWRITE` or a `BLOCK` — before `tool_permission`, `prerequisite`, `encoded_directive`, `data_flow`, or `confirmation` are reached. On a `BLOCK` this is safe by construction: the action does not proceed. On a `REWRITE` it is not: the redacted action is returned without re-checking whether the tool is in `allowed_tools`, whether a required predecessor succeeded, or whether a consequential action needs human confirmation. Concretely, a `payment_execute` lacking its `payment_confirm` predecessor, carrying a restricted-sensitivity token anywhere in its arguments, would be rewritten and released rather than blocked by `PREREQUISITE_NOT_MET` or escalated by `MISSING_CONFIRMATION`. This combination is not triggered by any of the 54 scenarios in the current library — `attack_success=0.0` across both splits — so it is not an observed failure. It is a residual architectural gap a Level-4 adaptive adversary would deliberately seek to combine: an action that is both out-of-policy *and* carries restricted content, so the first-matching rule short-circuits the policy checks that would otherwise apply.
